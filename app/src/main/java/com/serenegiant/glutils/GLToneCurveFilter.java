...
/**
 * Created by shunfujiwara on 15/08/13.
 */
@SuppressWarnings("WeakerAccess")
public class GLToneCurveFilter extends GLDrawer2D {

    private static final String TAG = GLToneCurveFilter.class.getSimpleName();

    private static int[] mToneCurveTexture = new int[]{-1};
    ...
    public static final String TONE_CURVE_FRAGMENT_SHADER = "" +
            ...
            " uniform samplerExternalOES toneCurveTexture;\n" +
            "\n" +
            " void main()\n" +
            " {\n" +
    ...
    protected void onInit(int programId) {
        ...
        mToneCurveTextureUniformLocation = GLES20.glGetUniformLocation(programId, "toneCurveTexture");
    ...
    protected void onDrawArraysPre() {
        if (mToneCurveTexture[0] != GLDrawer2D.NO_TEXTURE) {
            GLES20.glActiveTexture(GLES20.GL_TEXTURE3);
    ...
    private short readShort(InputStream input) throws IOException {
        short i = (short) (input.read() << 8 | input.read());
        return i;
    }
                ...
                if ((mRedCurve.size() >= 256) && (mGreenCurve.size() >= 256) && (mBlueCurve.size() >= 256)
                    ...
                    for (int currentCurveIndex = 0; currentCurveIndex < 256; currentCurveIndex++) {
                        ...
                        toneCurveByteArray[currentCurveIndex * 4 + 2] = (byte) ((int) Math.min(Math.max(
                                currentCurveIndex + mBlueCurve.get(currentCurveIndex) + mRgbCompositeCurve.get(
                                        currentCurveIndex), 0), 255) & 0xff);
                        toneCurveByteArray[currentCurveIndex * 4 + 1] = (byte) ((int) Math.min(Math.max(
                                currentCurveIndex + mGreenCurve.get(currentCurveIndex) + mRgbCompositeCurve.get(
                                        currentCurveIndex), 0), 255) & 0xff);
                        toneCurveByteArray[currentCurveIndex * 4] = (byte) ((int) Math.min(Math.max(
                                currentCurveIndex + mRedCurve.get(currentCurveIndex) + mRgbCompositeCurve.get(
                                        currentCurveIndex), 0), 255) & 0xff);
    ...
    private ArrayList<Point> createSplineCurve2(Point[] points) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            for (int x = cur.x; x < next.x; x++) {
                ...
                double y =
                        a * cur.y + b * next.y + (h * h / 6) * ((a * a * a - a) * sd[i] + (b * b * b - b) * sd[i
                                + 1]);
